...
class Single_Attention_layer(nn.Module):
    def __init__(self, config):
        ...
        # new: remember how many LapPE dims we have:
        #self.lap_dim       = config.lap_dim
        # convenience: total meta‐PE dims
        self.meta_dim      = self.num_metapaths * self.svd_dim

        # ──******* Eigenvalue‐Informed Gating: eigenvalues for LapPE dims ──
        # config.lap_eigvals is a torch.Tensor of shape (lap_dim,)
        ...
        # ── Per-head learnable scaling factor ──
       # self.head_scale = nn.Parameter(torch.full((1,), config.head_scale_init))  # one scalar per Single_Attention_layer instance
        ...
        #*********** Option 3 : Separate gating for MetaPE vs LapPE ********
        ...
        #self.gate_lap_Q  = nn.Linear(self.lap_dim,   self.lap_dim,   bias=True)
        ...
        #self.gate_lap_K  = nn.Linear(self.lap_dim,   self.lap_dim,   bias=True)
        # ***── Eigenvalue‐Informed: LapPE gate now accepts [LapPE; eigvals] → lap_dim
    ...
    def forward(self, input_x, pe_Q, pe_K):
        ...
        #g_meta_K = self.dropout_gate(torch.sigmoid(self.gate_meta_K(K_meta_n)))  # (N, meta_dim)


        #g_lap_K  = torch.sigmoid(self.gate_lap_K (K_lap_n))
        # ── Eigenvalue‐Informed LapPE gating ──
        # expand eigenvalues to (N, lap_dim) and concat to LapPE
        ...
        #g_lap_Q  = self.dropout_gate(torch.sigmoid(self.gate_lap_Q(lap_in_Q)))  # (N, lap_dim)
        g_lap_K    = torch.sigmoid(self.gate_lap_K(lap_in_K))
        #g_lap_K  = self.dropout_gate(torch.sigmoid(self.gate_lap_K(lap_in_K)))  # (N, lap_dim)
        #**************************************************#
...
#*******************************************************************
        ...
        #QKT = (Q @ K.t()) * self.head_scale

        QKT = F.dropout(QKT, p=self.dropout, training=self.training)
        # apply per-module attention dropout instead of global dropout
        #QKT = self.dropout_attn(QKT)
...
class GraphTransformerLayer(nn.Module):
    def __init__(self, config):
        ...
        self.heads = config.num_heads
        self.dropout = config.dropout
        # ── Per-module FFN dropout ──
        #self.dropout_ffn = nn.Dropout(config.dropout_ffn)
    ...
    def forward(self, x, pe_Q, pe_K, deg):
        ...
        # ── SAFE degree‐norm: add tiny ε so sqrt never zero ──
        # deg: (N,)  → we want (N,1) for broadcast
